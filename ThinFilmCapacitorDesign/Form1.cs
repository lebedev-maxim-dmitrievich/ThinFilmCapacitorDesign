...
namespace ThinFilmCapacitorDesign
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            textBox4.PlaceholderText = "fmax - max рабочая частота (кГц)";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Краевой эффект меньше единицы, невозможно спроектировать конденсатор. Необходимо выбрать другой материал, либо увеличить номинальную ёмкость", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return; // Прерывание выполнения метода
            }
            ...
            if (tgdelta < tgRAB)
            {
                MessageBox.Show("Рабочий тангенс угла диэлектрических потерь превышает заданный", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return; // Прерывание выполнения метода
            }
            ...
            if (Ysdop <= Ysrab)
            {
                MessageBox.Show("Погрешность активной площади конденсатора превышает допустимую", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return; // Прерывание выполнения метода
            }
